...
class _AnchorTargetLayer_FPN(nn.Module):
    ...
    """
    def __init__(self, feat_stride, scales, ratios, l_ratios):
        super(_AnchorTargetLayer_FPN, self).__init__()
        self._anchor_ratios = ratios
        self._anchor_l_ratios = l_ratios
    ...
    def forward(self, input):
        ...
        #   generate 27 anchor boxes centered on cell i
        #   apply predicted bbox deltas at cell i to each of the 27 anchors
        ...
        keep = ((anchors[:, 0] >= -self._allowed_border) &
                ...
                (anchors[:, 3] < long(im_info[0][2]) + self._allowed_border) &
                (anchors[:, 4] < long(im_info[0][1]) + self._allowed_border) &
                (anchors[:, 5] < long(im_info[0][0]) + self._allowed_border))
